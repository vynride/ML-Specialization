...
\quad w := w - \alpha \cdot \frac{\partial J(w, b)}{\partial w} \\
\quad b := b - \alpha \cdot \frac{\partial J(w, b)}{\partial b} \\
\text{\}}
\end{aligned}
```

-   $$w, b$$: The model parameters.
-   $$α$$: The **learning rate**, a positive scalar that controls the size of each step.
-   $$\frac{\partial J(w, b)}{\partial w}$$: The partial derivative (or gradient) of the cost function with respect to $$w$$. This tells us the slope of the cost function in the $$w$$ direction.
-   $$\frac{\partial J(w, b)}{\partial b}$$: The partial derivative of the cost function with respect to $$b$$.

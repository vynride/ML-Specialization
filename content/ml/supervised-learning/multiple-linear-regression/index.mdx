---
title: "🧠 Multiple Features & Vectorization"
order: 4
description: This section covers linear regression with multiple input variables, the concept of vectorization for computational efficiency, a practical guide to the NumPy library, and a hands-on lab implementation.
---
...
*   **$$\vec{w}$$**: The model's parameters (weights or coefficients), represented as a vector $$[w_1, w_2, ..., w_n]$$.
...
In this course, our training data `X\_train` will often be a 2-D array (matrix) of shape $$(m, n)$$, where $$m$$ is the number of training examples and $$n$$ is the number of features. When we process it example-by-example, we will index a single row, e.g., $$X[i]$$. This operation $$X[i]$$ returns a **1-D vector** of shape $$(n,)$$.

...
The model is a function, which we'll call $$f$$, that takes an input $$x$$ and produces a prediction $$\hat{y}$$ (pronounced "y-hat").

-   $$f$$: The model or hypothesis function.
-   $$\hat{y}$$: The predicted output from the model.
...
The cost function measures the difference (or error) between the model's predictions ($$\hat{y}$$) and the actual target values ($$y$$). For linear regression, the most common cost function is the **Squared Error Cost Function**.
